...
- `docker build -t tech-challenge-recomendacao .` — builda a imagem multi-stage (`Dockerfile`); não testado nesta máquina por falta de Docker instalado.
- `docker run --rm --env-file .env -v "$(pwd)/data:/app/data" -v "$(pwd)/models:/app/models" tech-challenge-recomendacao` — roda o stage `train` isolado no container.
