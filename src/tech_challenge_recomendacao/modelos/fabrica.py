...
def criar_modelo(
    tipo_modelo: str, n_usuarios: int, n_filmes: int, dimensao_embedding: int
) -> ModeloRecomendador:
    """Instancia um modelo de recomendação a partir do nome configurado.

    Args:
        tipo_modelo: Chave em `_MODELOS_DISPONIVEIS` (ex.: `"fatoracao_matricial"`).
    ...
    return classe_modelo(n_usuarios, n_filmes, dimensao_embedding)
